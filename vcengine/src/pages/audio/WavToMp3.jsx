...
export default function WavToMp3() {
  ...
  async function handleConvert() {
    ...
    try {
      ...
      const inputName = `input.${audioFile.name.split(".").pop() || "wav"}`;
      ...
      const fileOut = new File([blob], `converted.mp3`, { type: "audio/mpeg" });
    ...
    <div className="min-h-screen bg-[#0a0a0a] text-white">
      ...
      <main className="pt-28">
        <div className="mx-auto max-w-[1400px] px-6 pb-24">
          ...
          <div className="mb-12">
            <h1 className="text-6xl font-black uppercase">Wav to Mp3</h1>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[1fr_340px] gap-8 items-start">
            <div className="space-y-6">
              {!audioFile ? <AudioUploadZone onFilesSelected={handleFileSelected} accept="audio/wav,.wav" /> : <div className="border-[3px] border-black bg-[#161616] shadow-[6px_6px_0px_0px_black] p-6 text-center"><p className="font-mono text-sm font-bold">{audioFile.name}</p></div>}
          ...
          {result && (
            <div className="mt-10">
              <AudioResultCard file={result.file} url={result.url} stats={result.stats} downloadLabel="Download MP3" onDownload={handleDownload} onClear={() => setResult(null)} />
            </div>
          )}
        </div>
      </main>
