...
export async function reverseAudio({ file, outputFormat = "mp3", quality = "high", onProgress }) {
  ...
  const fileOut = new File([blob], `reversed-${file.name.replace(/\.[^/.]+$/, "")}.${fmt.ext}`, { type: fmt.mime });
