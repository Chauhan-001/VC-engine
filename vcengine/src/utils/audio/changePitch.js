...
export async function changePitch({ file, pitch, outputFormat = "mp3", quality = "high", onProgress }) {
  ...
  const cmd = [
    "-i", inputName,
    "-filter:a", `asetrate=44100*${pitch},aresample=44100`,
    "-c:a", fmt.codec,
  ];
  ...
  const fileOut = new File([blob], `pitch-${pitch}x-${file.name.replace(/\.[^/.]+$/, "")}.${fmt.ext}`, { type: fmt.mime });
