...
export async function changeVolume({ file, volume, outputFormat = "mp3", quality = "high", onProgress }) {
  ...
  const fileOut = new File([blob], `volume-${file.name.replace(/\.[^/.]+$/, "")}.${fmt.ext}`, { type: fmt.mime });
